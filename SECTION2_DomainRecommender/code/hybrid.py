...
def load_all_data():
    ...
    # Create punk metadata from items
    all_items = pd.concat([train_df, test_df])['item_id'].unique()
    punks = pd.DataFrame({
        'item_id': all_items,
    ...
    return train_df, test_df, punks, user_map, item_map
...
def predict_hybrid_batch(
    ...
    for idx, row in test_df.iterrows():
        ...
        if user_profile is not None and item_id in item_id_to_idx_cb:
            ...
            # Normalize to 0-1
            cb_score = max(0.0, min(1.0, float(cb_score)))
...
if __name__ == "__main__":
    ...
    tfidf_matrix, item_id_to_idx_cb, item_ids_cb = build_item_features(punks_df, use_id=True)
