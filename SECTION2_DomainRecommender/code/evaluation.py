...
def evaluate_content_based(train_df, test_df, punks_df):
...
def evaluate_hybrid(train_df, test_df, punks_df, user_map, item_map, alpha=0.5, k_cf=20):
...
def cold_start_analysis(train_df, test_df, punks_df, user_map, item_map, threshold=5):
    ...
    if len(cold_start_df) > 0:
        print("\n--- Cold-Start Users ---")
        cb_result = evaluate_content_based(train_df, cold_start_df, punks_df)
        if cb_result:
        ...
        hybrid_result = evaluate_hybrid(train_df, cold_start_df, punks_df, user_map, item_map, alpha=0.7, k_cf=20)
        if hybrid_result:
    ...
    if len(non_cold_start_df) > 0:
        ...
        cb_result = evaluate_content_based(train_df, non_cold_start_df, punks_df)
        if cb_result:
        ...
        hybrid_result = evaluate_hybrid(train_df, non_cold_start_df, punks_df, user_map, item_map, alpha=0.3, k_cf=20)
        if hybrid_result:
...
def compare_models(train_df, test_df, punks_df, user_map, item_map):
    """Compare all models and generate plots."""
    print("=" * 60)
    print("COMPREHENSIVE MODEL EVALUATION (CryptoPunk)")
    print("=" * 60)
    
    results = []
    
    # Content-based
    cb_result = evaluate_content_based(train_df, test_df, punks_df)
    if cb_result:
        results.append(cb_result)
    ...
    for alpha in [0.3, 0.5, 0.7]:
        hybrid_result = evaluate_hybrid(train_df, test_df, punks_df, user_map, item_map, alpha=alpha, k_cf=20)
        if hybrid_result:
            results.append(hybrid_result)
...
if __name__ == "__main__":
    ...
    # Full comparison
    results_df = compare_models(train_df, test_df, punks_df, user_map, item_map)
    
    # Cold-start analysis
    cold_start_results = cold_start_analysis(train_df, test_df, punks_df, user_map, item_map, threshold=5)
